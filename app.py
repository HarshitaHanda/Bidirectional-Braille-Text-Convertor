...
# ----------------------- Capture Number Pad Input -----------------------
def capture_braille_input():
    # Use the laptop's number pad (keys 1 to 9)
    braille_map = {
        '7': '⠧',  # U
        '8': '⠫',  # V
        '4': '⠍',  # M
    ...
    # Listen to the key press from number pad (7, 8, 4, 5, 1, 2)
    key_pressed = st.text_input("Press a key on the number pad (7, 8, 4, 5, 1, 2):", "")
    
    if key_pressed in braille_map:
        braille_input.append(braille_map[key_pressed])
        st.session_state.input_text = ''.join(braille_input)
...
braille_input = capture_braille_input()
...
# Input Text (automatically updates with the number pad input)
input_text = st.text_area("Input Text", height=150, value=st.session_state.input_text, key="input_area")
st.session_state.input_text = input_text  # Sync input_text with session state
